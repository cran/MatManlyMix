...
double mGloglik_Manly_Full(int p, int T, int n, int K, double ***Y, double **la, double *tau, double ***Mu, double ***invS, double ***invPsi, double *detS, double *detPsi){
	...
	for(i=0; i<n; i++){	 
		...
		for(k=0; k<K; k++){
			...
			dens = mGpdf_Manly_Full(p, T, la[k], Yi, Muk, invSk, invPsik, detS[k], detPsi[k]);
...
double Q(int n, int p, int T, double *la_nonzero, int *index, double ***Y, double *gamma_k, double **invSk){
	...
	int i, j, t, t1, t2, count;
	double res, jac, sum_gamma, det1, det2;
	double *la;
	double **Psik, **temp1, **temp2;
	double ***MY, **Muk, *Eig1, *Eig2, **MYi, **tMYi, **invSk0;
	...
	for(i=0; i<n; i++){
		...
		multiply(tMYi, T, p, invSk0, p, p, temp1);
		...
		for(t1=0; t1<T; t1++){			
			...
			for(t2=0; t2<T; t2++){
				...
				Psik[t1][t2] += gamma_k[i] * temp2[t1][t2] / p /sum_gamma;
	...
	#ifdef __HAVE_R_
		EigValDec(T, Eig1, Psik, &det1);
	#else
		cephes_symmeigens_down(T, Eig1, Psik, &det1);
	#endif
...
double Mstep_Manly_Full(int p, int T, int n, int K, double *misc_double, double ***Y, double **la, double **gamma, double ***invS, double ***Mu, double ***invPsi, double *detS, double *detPsi, double *tau){
	...
	for(k=0; k<K; k++){
		...
		if(sum_index > 0){
			...
			min_value = simplex(Q, n, p, T, index, Y, gamma_k, invSk, la_nonzero, eps, 0.1);
			...
			Q_value[k] = Q(n, p, T, la_nonzero, index, Y, gamma_k, invSk);
		...
		for(i=0; i<n; i++){
			...
			for(t1=0; t1<T; t1++){			
				...
				for(t2=0; t2<T; t2++){
					...
					Psi[t1][t2] += gamma[i][k] * temp2[t1][t2] / p /sum_gamma[k];
...
void EM_Manly_Full(int p, int T, int n, int K, double ***Y, double **la, int max_iter, double *misc_double, double *tau, double ***Mu, double ***invS, double ***invPsi, double *detS, double *detPsi, double **gamma, int *id, double *ll, int *conv){
	...
	do{
		...
		Estep_Manly_Full(p, T, n, K, Y, la, tau, Mu, invS, invPsi, detS, detPsi, gamma);

		loglik_old = Mstep_Manly_Full(p, T, n, K, misc_double, Y, la, gamma, invS, Mu, invPsi, detS, detPsi, tau);
	...
	ll[0] = mGloglik_Manly_Full(p, T, n, K, Y, la, tau, Mu, invS, invPsi, detS, detPsi);
...
double mGloglik_Manly_Reg(int p, int T, int n, int q, int K, double ***Y, double **la, double *tau, double ***X, double ***beta, double ***invS, double ***invPsi, double *detS, double *detPsi){
	...
	for(i=0; i<n; i++){	 
		...
		for(k=0; k<K; k++){
			...
			dens = mGpdf_Manly_Reg(p, T, q, la[k], Yi, Xi, betak, invSk, invPsik, detS[k], detPsi[k]);
...
double Mstep_Manly_Reg(int p, int T, int n, int q, int K, double *misc_double, double ***Y, double **la, double **gamma, double ***invS, double ***X, double ***beta, double ***invPsi, double *detS, double *detPsi, double *tau){
	...
	int i,j,j1,j2,l1, l2, l, k,t,t1,t2, sum_index, count, *index;
	...
	for(k=0; k<K; k++){
		...
		if(sum_index > 0){
			...
			min_value = simplex_Reg(Q_Reg, n, p, T, q, index, Y, gamma_k, invPsik, X, la_nonzero, eps, 0.1);
			//printf(" M step %d %lf \n", K, min_value);
			...
			Q_value[k] = Q_Reg(n, p, T, q, la_nonzero, index, Y, gamma_k, invPsik, X);
...
void EM_Manly_Reg(int p, int T, int n, int q, int K, double ***Y, double **la, int max_iter, double *misc_double, double *tau, double ***X, double ***beta, double ***invS, double ***invPsi, double *detS, double *detPsi, double **gamma, int *id, double *ll, int *conv){
	...
	do{
		...
		Estep_Manly_Reg(p, T, n, q, K, Y, la, tau, X, beta, invS, invPsi, detS, detPsi, gamma);

		loglik_old = Mstep_Manly_Reg(p, T, n, q, K, misc_double, Y, la, gamma, invS, X, beta, invPsi, detS, detPsi, tau);
							
	}

	while ((iter < max_iter) && (fabs(loglik - loglik_old) / fabs(loglik_old) > eps));

	ll[0] = mGloglik_Manly_Reg(p, T, n, q, K, Y, la, tau, X, beta, invS, invPsi, detS, detPsi);
	conv[0] = iter;
	if(fabs(loglik - loglik_old) / fabs(loglik_old) <= eps){
		conv[1] = 0;
	} else{
